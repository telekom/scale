...
describe('Search', () => {
  ...
  beforeEach(async () => {
    page = await newSpecPage({
      components: [Search],
      html: `<scale-search label="test"></scale-search>`,
    });
  });
  ...
  it('should handle css classes', () => {
    const element = new Search();

    element.hasFocus = true;
    expect(element.getCssClassMap()).toContain('search--has-focus');

    element.disabled = true;
    expect(element.getCssClassMap()).toContain('search--disabled');

    element.transparent = true;
    expect(element.getCssClassMap()).toContain('search--transparent');
  });
  ...
  it('should emit Onchange only once', async () => {
    const controlledInputPage = await newSpecPage({
      components: [Search],
      html: `<scale-search controlled></scale-search>`,
    });
    const inputSpy = jest.fn();
    const inputSpyLegacy = jest.fn();
