import { LengthValidator, TextValidator, NumberValidator, EmailValidator } from './validators';

const resultFormat = (result: any) => `Should return ${result.result} for "${result.value}"`

describe('LengthValidator', () => {
	const results: Array<{ min: number, max: number, value: string, result: boolean }> = [
	...
	results.forEach(result =>
		it(`${resultFormat(result)} with min: ${result.min} and max: ${result.max}`, () => {
			expect(LengthValidator(result.min, result.max).validate(result.value)).toEqual(result.result)
		})
	)
});
