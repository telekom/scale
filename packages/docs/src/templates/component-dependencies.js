...
export const ComponentDependencies = ({ componentDocs }) => {
  ...
  const hasDependencies =
    componentDocs && dependencies && dependencies.length !== 0
  const hasDependencyGraph =
    componentDocs && dependencyGraph && JSON.stringify(dependencyGraph) !== "{}"
    ...
    <div className="components__dependencies">
      ...
      {hasDependents && (
        <div>
          ...
          {dependents.map((dependent, index) => (
            <div key={index}>
              <a href={`../${dependent.replace("scale-", "")}`}>{dependent}</a>
            </div>
          ))}
        </div>
      )}
      ...
      {hasDependencyGraph && (
        <div>
          ...
          <div>{JSON.stringify(dependencyGraph, null, 4)}</div>
        </div>
      )}
    </div>
  )
