  ...
  parameters={{
    docs: {
      container: ({ context }) => {
        ...
        let levelStringItem = [];
        let currentSection;
        let tempCurrentSubSection;
        let contrastSections;
        let sectionsWithSubtitle = ['UI', 'Additional', 'Functional'];
        ...
        function isNewSection(jsonPosition) {
          if (jsonPosition !== 0) {
            if (
              allTokens.tokens[jsonPosition - 1].section !==
              allTokens.tokens[jsonPosition].section
            ) {
              newSection = true;
              currentSection = allTokens.tokens[jsonPosition].section;
            } else {
              newSection = false;
            }
          } else {
        ...
        function isNewSubSection(jsonPosition) {
          if (jsonPosition !== 0) {
            let lastSubsection = allTokens.tokens[jsonPosition - 1].name.split(
              ' / '
            )[1];
            let currentSubsection = allTokens.tokens[jsonPosition].name.split(
              ' / '
            )[1];
            if (lastSubsection !== currentSubsection) {
              newSubSection = true;
              tempCurrentSubSection = allTokens.tokens[jsonPosition].name.split(
                ' / '
              )[1];
            } else {
              newSubSection = false;
        ...
        function checkForWantedSubTitles() {
          for (let i = 0; i < sectionsWithSubtitle.length; i++) {
            if (currentSection === sectionsWithSubtitle[i]) {
              return true;
            }
          }
        }
        ...
        function createContrastObjectArray() {}
        return (
            ...
            <table style={{ backgroundColor: `${darkMode ? '#000' : '#fff'}` }}>
              {allTokens.tokens
                ...
                .map((item, i) => {
                  ...
                  {
                    item.contrastChecks
                      ? (pathStringItem = item.contrastChecks.map(
                          (contrastCkeckItem) => {
                            return contrastCkeckItem.path;
                          }
                        ))
                      : null;
                  }
                  {
                    item.contrastChecks
                      ? (levelStringItem = item.contrastChecks.map(
                          (contrastCkeckItem) => {
                            return (
                              contrastCkeckItem.path +
                              '/' +
                              contrastCkeckItem.level
                            );
                          }
                        ))
                      : null;
                  ...
                  return (
                    <div>
                      ...
                      <tbody>
                        {newSubSection && checkForWantedSubTitles() ? (
                          <div
                            style={{
                              marginTop: '50px',
                              marginLeft: '10px',
