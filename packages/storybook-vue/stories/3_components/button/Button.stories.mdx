...
export const Template = (args, { argTypes }) => ({
    ...
    >
      ...
      <scale-icon-action-add accessibility-title="Add" v-if="iconOnly" :size="size == 'small' ? 16 : 24" />
      <scale-icon-navigation-right accessibility-title="Next" v-if="icon === 'after'" :size="size == 'small' ? 16 : 24" />
...
<scale-button href="https://example.com" target="_blank">
  ...
  <scale-icon-navigation-external-link accesibility-title="External link, opens in new tab" size="24"></scale-icon-navigation-external-link>
</scale-button>
```
