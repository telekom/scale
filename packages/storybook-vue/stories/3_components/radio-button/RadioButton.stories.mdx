import {
  Meta,
  Story,
  ArgsTable,
  Canvas,
  Description,
} from '@storybook/addon-docs/blocks';
import ScaleRadioButton from './ScaleRadioButton.vue';
import { action } from '@storybook/addon-actions';
export const ICON =
  'M16.471,8.321 L12.613594,0.502886112 L8.755,8.321 L0.126435258,9.57533801 L6.37,15.66 L4.89610548,24.2548735 L12.613,20.197 L20.3310826,24.2548735 L18.856,15.66 L25.1007528,9.57533801 L16.471,8.321 Z M9.41891527,9.23556376 L12.613,2.761 L15.8082728,9.23556376 L22.951,10.273 L17.7826928,15.3122039 L19.003,22.427 L12.613594,19.067774 L6.223,22.427 L7.44449521,15.3122039 L2.275,10.273 L9.41891527,9.23556376 Z';

<Meta
  title="Components/Radio Button"
  component={ScaleRadioButton}
  argTypes={{}}
/>

export const Template = (args, { argTypes }) => ({
  components: { ScaleRadioButton },
  props: {
    label: String,
    ...ScaleRadioButton.props,
  },
  template: `
    <scale-radio-button
      :status="status"
      :name="name"
      :label="label"
      :helper-text="helperText"
      :disabled="disabled"
      :checked="checked"
      :value="value"
      :input-id="inputId"
      @scaleChange="scaleChange"
    ></scale-radio-button>
  `,
  methods: {
    scaleChange: action('scaleChange'),
  },
});

<div
  style={{
    display: 'inline-flex',
    alignItems: 'center',
    justifyContent: 'space-between',
    width: '100%',
  }}
>
  <h1>Radio Button</h1>
  <img src="assets/aa.png" alt="Accessible AA" />
</div>

## Standard

<Canvas withSource="none">
  <Story
    name="Standard"
    args={{
      label: 'Radio',
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

<ArgsTable story="Standard" />

### Scoped CSS variables

```css
scale-radio-button {
  --color-disabled: var(--scl-color-background-disabled);
  --background-disabled: var(--scl-color-white);
  --color-error: var(--scl-color-text-error);
  --transition: all var(--scl-motion-duration-fast) var(
      --scl-motion-easing-standard
    );
  --color-primary: var(--scl-color-primary, #e20074);
  --color-primary-hover: var(--scl-color-primary-hover, #f90984);
  --color-primary-active: var(--scl-color-primary-active);
  --color-focus: var(--scl-color-focus);
  --color-text: var(--scl-color-text-standard);

  /*helper-text*/
  --transition-helper-text: var(--transition);
  --font-size-helper-text: var(--scl-font-size-12);
  --font-weight-helper-text: var(--scl-font-weight-bold);
  --line-height-helper-text: var(--scl-font-line-height-133);
  --color-helper-text: var(--scl-color-blue-70);
  --spacing-left-helper-text: var(--scl-spacing-24);
  --spacing-top-helper-text: var(--scl-spacing-4);

  /*label*/
  --transition-label: var(--transition);
  --font-weight-label: var(--scl-font-weight-medium);
  --color-label: var(--color-text);

  /*control*/
  --width-control: var(--scl-spacing-16);
  --height-control: var(--scl-spacing-16);
  --transition-control: var(--transition);
  --spacing-control: 0 var(--scl-spacing-8) 0 0;
  --background-color-control: var(--scl-color-white);
  --border-control: var(--scl-spacing-1) solid var(--color-text);
  --border-control-checked: calc(0.5 * var(--scl-spacing-12)) solid var(
      --color-primary
    );
  --border-control-checked-active: calc(0.5 * var(--scl-spacing-12)) solid var(
      --color-primary-active,
      #cb0068
    );
  --border-control-checked-disabled: calc(0.5 * var(--scl-spacing-12)) solid var(
      --color-disabled
    );
  --background-control-checked-disabled: var(--scl-color-grey-30);
  --border-control-active: var(--scl-spacing-8) solid var(
      --color-primary-active
    );
  --border-control-disabled: var(--scl-spacing-1) solid var(--color-disabled);
  --border-control-error: var(--scl-spacing-2) solid var(
      --scl-color-background-error
    );
  --box-shadow-control-focus: 0 0 0 var(--scl-spacing-2) var(--color-focus);
}
```

## Standard Disabled

<Canvas withSource="none">
  <Story
    name="Standard Disabled"
    args={{
      label: 'Standard Disabled',
      disabled: true,
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-radio-button disabled @scaleChange="handleChange"></scale-radio-button>
```

## Selected

<Canvas withSource="none">
  <Story
    name="Selected"
    args={{
      label: 'Selected',
      checked: true,
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-radio-button checked @scaleChange="handleChange"></scale-radio-button>
```

## Selected Disabled

<Canvas withSource="none">
  <Story
    name="Selected Disabled"
    args={{
      label: 'Selected Disabled',
      checked: true,
      disabled: true,
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-radio-button
  checked
  disabled
  @scaleChange="handleChange"
></scale-radio-button>
```

## Error

<Canvas withSource="none">
  <Story
    name="Error"
    args={{
      label: 'Standard with error',
      status: 'error',
      helperText: 'This is the error message',
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-radio-button
  status="error"
  helper-text="This is the error message"
  @scaleChange="handleChange"
></scale-radio-button>
```
