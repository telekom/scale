import { Meta, Story, ArgsTable, Canvas } from '@storybook/addon-docs/blocks';
import ScaleToggleGroup from './ScaleToggleGroup.vue';

<Meta
  title="Beta Components/Toggle Group"
  component={ScaleToggleGroup}
  argTypes={{
    variant: {
      control: { type: 'select', options: ['grey-background', 'white-background'] },
    },
    size: {
      control: { type: 'select', options: ['xs', 'small', 'regular', 'large'] },
    },
	  fullWidth: {
      control: { type: 'boolean'},
    },
    ariaLabelTranslation: {
      control: { type: "text", },
    },
    colorScheme: {
      control: { type: 'select', options: ['light', 'dark'] },
    },
  }}
/>

export const Template = (args, { argTypes }) => ({
  components: { ScaleToggleGroup },
  props: {
    label: String,
    icon: String,
    ...ScaleToggleGroup.props,
  },
  template: `
    <scale-toggle-group
      :size="size"
      :variant="variant"
      :full-width="fullWidth"
      :disabled="disabled"
      :hide-border="hideBorder"
      :single-select="singleSelect"
      :aria-label-translation="ariaLabelTranslation"
      :color-scheme="colorScheme"
      :styles="styles"
      @scaleChange="scaleChange"
    >
      <scale-toggle-button>Label</scale-toggle-button>
      <scale-toggle-button>Label</scale-toggle-button>
      <scale-toggle-button selected>Label</scale-toggle-button>
    </scale-toggle-group>
  `,
});

export const TemplateAriaTranslation = (args, { argTypes }) => ({
  components: { ScaleToggleGroup },
  props: {
    label: String,
    icon: String,
    ...ScaleToggleGroup.props,
  },
  template: `
    <scale-toggle-group
      :size="size"
      :variant="variant"
      :full-width="fullWidth"
      :disabled="disabled"
      :hide-border="hideBorder"
      :single-select="singleSelect"
      :aria-label-translation="ariaLabelTranslation"
      :color-scheme="colorScheme"
      :styles="styles"
      @scaleChange="scaleChange"
    >
      <scale-toggle-button 
        aria-label="Drück mich"
        aria-lang-selected="selektiert"
        aria-lang-deselected="deselektiert">Label
      </scale-toggle-button>
      <scale-toggle-button 
        aria-label="Drück mich"
        aria-lang-selected="selektiert"
        aria-lang-deselected="deselektiert">Label
      </scale-toggle-button>
      <scale-toggle-button 
        aria-label="Drück mich"
        aria-lang-selected="selektiert"
        aria-lang-deselected="deselektiert"
        selected
        >Label
      </scale-toggle-button>
    </scale-toggle-group>
  `,
});

export const TemplateIcons = (args, { argTypes }) => ({
  components: { ScaleToggleGroup },
  props: {
    label: String,
    iconBefore: Boolean,
    ...ScaleToggleGroup.props,
  },
  template: `
    <scale-toggle-group
      :size="size"
      :variant="variant"
      :full-width="fullWidth"
      :disabled="disabled"
      :hide-border="hideBorder"
      :single-select="singleSelect"
      :aria-label-translation="ariaLabelTranslation"
      :color-scheme="colorScheme"
      :styles="styles"
      @scaleChange="scaleChange"
    >
      <scale-toggle-button v-if="iconBefore">
        <scale-icon-action-search /> Label
      </scale-toggle-button>
      <scale-toggle-button icon-only="true" v-if="!iconBefore">
        <scale-icon-action-search />
      </scale-toggle-button>
      <scale-toggle-button v-if="iconBefore">
        <scale-icon-action-search /> Label
      </scale-toggle-button>
      <scale-toggle-button icon-only="true" v-if="!iconBefore">
        <scale-icon-action-search />
      </scale-toggle-button>
      <scale-toggle-button selected v-if="iconBefore">
        <scale-icon-action-search /> Label
      </scale-toggle-button>
      <scale-toggle-button icon-only="true" selected  v-if="!iconBefore">
        <scale-icon-action-search />
      </scale-toggle-button>
    </scale-toggle-group>
  `,
});

<div style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'space-between', width: '100%'}}>
    <h1>Toggle Group</h1>
    <img src="assets/beta.png" alt="Beta Component" />
</div>

## Standard

<Canvas withSource="none">
  <Story
    name="Standard"
    args={{}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

<ArgsTable story="Standard" />

```html
<scale-toggle-group>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## White Background

<Canvas withSource="none">
  <Story
    name="White Background"
    args={{
		  variant: 'whiteBackground'
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group variant='white-background'>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Single Select

<Canvas withSource="none">
  <Story
    name="Single Select"
    args={{
		  singleSelect: true
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group single-select>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Dark Color Scheme

<Canvas withSource="none">
  <Story
    name="Dark Color Scheme"
    args={{
      colorScheme: 'dark'
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group color-scheme="dark">
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Dark Color Scheme White Background

<Canvas withSource="none">
  <Story
    name="Dark Color Scheme White Background"
    args={{
      colorScheme: 'dark',
      variant: 'white-background'
    }}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group color-scheme="dark" variant="white-background">
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Disabled

<Canvas withSource="none">
  <Story
    name="Disabled"
    args={{
		  disabled: true
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group disabled>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Disabled White Background

<Canvas withSource="none">
  <Story
    name="Disabled White Background"
    args={{
		  disabled: true,
		  variant: 'whiteBackground'
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group variant='white-background' disabled>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Regular Size

<Canvas withSource="none">
  <Story
    name="Regular Size"
    args={{
		  size: 'regular'
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="regular">
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Small Size Block

<Canvas withSource="none">
  <Story
    name="Small Size Block"
    args={{
		  size: 'small',
		  fullWidth: true,
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="small" full-width>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## XS Size Block

<Canvas withSource="none">
  <Story
    name="XS Size Block"
    args={{
		  size: 'xs',
		  fullWidth: true,
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="xs" full-width>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## No Border

<Canvas withSource="none">
  <Story
    name="No Border"
    args={{
		  hideBorder: true
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group hide-border>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Disabled, White Background, Regular

<Canvas withSource="none">
  <Story
    name="Disabled, White Background, Regular"
    args={{
		  disabled: true,
		  variant: 'whiteBackground',
		  size: 'regular'
	}}
  >
    {Template.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group disabled variant="white-background" size="regular">
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button>Label</scale-toggle-button>
  <scale-toggle-button selected>Label</scale-toggle-button>
</scale-toggle-group>
```

## Accessibility, German

<Canvas withSource="none">
  <Story
    name="Accessibility, German"
    args={{
      ariaLabelTranslation: "wechsel schalter gruppe mit $slottedButtons schaltern"
    }}
  >
    {TemplateAriaTranslation.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group aria-label-translation="wechsel schalter gruppe mit $slottedButtons schaltern">
  <scale-toggle-button 
    aria-label="Drück mich"
    aria-lang-selected="selektiert"
    aria-lang-deselected="deselektiert">Label
  </scale-toggle-button>
  <scale-toggle-button 
    aria-label="Drück mich"
    aria-lang-selected="selektiert"
    aria-lang-deselected="deselektiert">Label
  </scale-toggle-button>
  <scale-toggle-button 
    aria-label="Drück mich"
    aria-lang-selected="selektiert"
    aria-lang-deselected="deselektiert"
    selected
    >Label
  </scale-toggle-button>
</scale-toggle-group>
```

## Icon Before Large

<Canvas withSource="none">
  <Story
    name="Icon Before Large"
    args={{
      iconBefore: true
    }}
  >
    {TemplateIcons.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group>
	<scale-toggle-button>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button selected>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
</scale-toggle-group>
```

## Icon Only Large

<Canvas withSource="none">
  <Story
    name="Icon Only Large"
    args={{
      iconBefore: false
    }}
  >
    {TemplateIcons.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group>
	<scale-toggle-button icon-only>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button icon-only>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button icon-only selected>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
</scale-toggle-group>
```

## Icon Before Regular

<Canvas withSource="none">
  <Story
    name="Icon Before Regular"
    args={{
      iconBefore: true,
      size: 'regular'
    }}
  >
    {TemplateIcons.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="regular">
	<scale-toggle-button>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button selected>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
</scale-toggle-group>
```

## Icon Only Regular

<Canvas withSource="none">
  <Story
    name="Icon Only Regular"
    args={{
      iconBefore: false,
      size: 'regular'
    }}
  >
    {TemplateIcons.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="regular">
	<scale-toggle-button icon-only>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button icon-only>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button icon-only selected>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
</scale-toggle-group>
```

## Icon Before Small White-Background

<Canvas withSource="none">
  <Story
    name="Icon Before Small White-Background"
    args={{
      iconBefore: true,
      size: 'small',
      variant: 'whiteBackground',
    }}
  >
    {TemplateIcons.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="small" variant="white-background">
	<scale-toggle-button>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button selected>
    Label <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
</scale-toggle-group>
```

## Icon Only Small White-Background

<Canvas withSource="none">
  <Story
    name="Icon Only Small White-Background"
    args={{
      iconBefore: false,
      size: 'small',
      variant: 'whiteBackground',
    }}
  >
    {TemplateIcons.bind({})}
  </Story>
</Canvas>

```html
<scale-toggle-group size="small" variant="white-background">
	<scale-toggle-button icon-only>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button icon-only>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
  <scale-toggle-button icon-only selected>
    <scale-icon-action-search></scale-icon-action-search>
  </scale-toggle-button>
</scale-toggle-group>
```
