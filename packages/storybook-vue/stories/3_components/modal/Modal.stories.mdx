...
<scale-modal
  ...
  :opened="opened"
  @scaleClose="opened = false"
>
  <p>Hello. Welcome. What a pleasure it is to have you.</p>
  <scale-button variant="secondary" @click="opened = false">
    Cancel
  </scale-button>
  <scale-button @click="handlePrimaryAction"> Primary Action </scale-button>
</scale-modal>
```
...
:host {
  ...
  --border-bottom-header-has-scroll: var(--scl-spacing-1) solid var(
      --scl-color-grey-20
    );
  ...
  --transition-close-button: all var(--scl-motion-duration-fast) var(
      --scl-motion-easing-standard
    );
  --box-shadow-close-button-focus: 0 0 0 var(--scl-spacing-2) var(
      --scl-color-focus
    );
